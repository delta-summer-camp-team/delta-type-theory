 ...
 */
fun whnf(term: CoreTerm, resolve: GlobalResolver): CoreTerm = when (term) {
  is TypeTerm -> term
  is BoundVar -> term
  ...
  is App -> when (val f = whnf(term.function, resolve)) {
 ...
 */
fun normalize(term: CoreTerm, resolve: GlobalResolver): CoreTerm = when (val head = whnf(term, resolve)) {
  is Pi -> Pi(
 ...
 */
fun definitionallyEqual(left: CoreTerm, right: CoreTerm, resolve: GlobalResolver): Boolean =
  normalize(left, resolve) == normalize(right, resolve)
...
fun whnf(term: CoreTerm, ctx: ElaborationContext): CoreTerm =
  whnf(term) { name -> ctx.lookupGlobal(name.value)?.value }

fun normalize(term: CoreTerm, ctx: ElaborationContext): CoreTerm =
  normalize(term) { name -> ctx.lookupGlobal(name.value)?.value }

fun definitionallyEqual(left: CoreTerm, right: CoreTerm, ctx: ElaborationContext): Boolean =
  definitionallyEqual(left, right) { name -> ctx.lookupGlobal(name.value)?.value }
 ...
 */
private fun CoreTerm.substitute(index: Int, replacement: CoreTerm, depth: Int = 0): CoreTerm = when (this) {
  is BoundVar -> when {
